...
int test_probe_creation_and_insert() {
    ...
    // Initially empty
    if (mgr.count() != 0) return fail("expected 0 probes initially");
    ...
    if (idx1 != 0) return fail("first probe should get index 0");
    if (mgr.count() != 1) return fail("expected 1 probe after insert");
    ...
    if (idx2 != 1) return fail("second probe should get index 1");
    if (mgr.count() != 2) return fail("expected 2 probes after second insert");
    ...
    if (!near_eq(r1.influence_radius, 20.0F)) return fail("probe 0 radius mismatch");
    if (r1.cubemap.id != 42U) return fail("probe 0 cubemap handle mismatch");
    ...
    if (!near_eq(r2.intensity, 0.5F)) return fail("probe 1 intensity mismatch");
...
int test_probe_clear() {
    ...
    mgr.add_probe(p);
    mgr.add_probe(p);
    if (mgr.count() != 3) return fail("expected 3 probes before clear");

    mgr.clear();
    if (mgr.count() != 0) return fail("expected 0 probes after clear");

    // Query after clear should be empty
    float pos[3] = {1.0F, 2.0F, 3.0F};
    auto res = mgr.find_probes_near(pos);
    if (!res.empty()) return fail("query after clear should be empty");
...
int test_probe_remove() {
    ...
    mgr.add_probe(p);
    mgr.add_probe(p);
    if (mgr.count() != 3) return fail("expected 3 probes");

    mgr.remove_probe(1);
    if (mgr.count() != 2) return fail("expected 2 probes after remove");

    // Remaining probes should be indices 0 and 1 (was 0 and 2)
    const auto& r0 = mgr.get_probe(0);
    if (!near_eq(r0.position[0], 0.0F)) return fail("first remaining probe wrong");
    const auto& r1 = mgr.get_probe(1);
    if (!near_eq(r1.position[0], 0.0F)) return fail("second remaining probe wrong");
...
int main() {
    if (int rc = test_probe_creation_and_insert(); rc != 0) return rc;
    if (int rc = test_probe_find_near(); rc != 0) return rc;
    if (int rc = test_probe_aabb_overlap(); rc != 0) return rc;
    if (int rc = test_probe_clear(); rc != 0) return rc;
    if (int rc = test_probe_remove(); rc != 0) return rc;
