...
int test_ssaopass_default_construction() {
    ...
    // Default parameters
    if (!near_eq(pass.radius, 0.5F))
        return fail("default radius should be 0.5");
    if (!near_eq(pass.power, 2.0F))
        return fail("default power should be 2.0");
    if (!near_eq(pass.bias, 0.025F))
        return fail("default bias should be 0.025");
    return 0;
}
...
int test_ssaopass_parameter_setters() {
    ...
    if (!near_eq(pass.radius, 1.0F))
        return fail("radius setter failed");
    if (!near_eq(pass.power, 1.5F))
        return fail("power setter failed");
    if (!near_eq(pass.bias, 0.01F))
        return fail("bias setter failed");
    return 0;
}
...
int test_ssaopass_ao_texture_default() {
    ...
    if (tex.id != 0)
        return fail("ao_texture should be 0 before init");
    return 0;
}
...
int test_taa_default_construction() {
    ...
    if (taa.prev_view() != nullptr)
        return fail("prev_view should be null before init");
    if (taa.prev_proj() != nullptr)
        return fail("prev_proj should be null before init");
    return 0;
}
...
int test_taa_initialize_shutdown() {
    ...
    if (!ok)
        return fail("initialize should succeed");
    taa.shutdown();
...
int test_taa_jitter_repeatable() {
    ...
    if (jitter1[0] != jitter2[0])
        return fail("jitter x should be repeatable for same frame");
    if (jitter1[1] != jitter2[1])
        return fail("jitter y should be repeatable for same frame");

    for (int i = 0; i < 16; ++i) {
        if (!near_eq(out1[i], out2[i]))
            return fail("jittered projection should be repeatable");
    }
...
int test_taa_jitter_only_affects_projection_xy() {
    ...
    for (int i = 0; i < 16; ++i) {
        if (i == 8 || i == 9)
            continue;
        if (!near_eq(out[i], base_proj[i]))
            return fail("jitter should only affect elements 8 and 9");
    }
...
int test_taa_store_prev_matrices() {
    ...
    if (taa.prev_view() == nullptr)
        return fail("prev_view should not be null after store");
    if (taa.prev_proj() == nullptr)
        return fail("prev_proj should not be null after store");

    for (int i = 0; i < 16; ++i) {
        if (!near_eq(taa.prev_view()[i], view[i]))
            return fail("prev_view mismatch at index " + std::to_string(i));
        if (!near_eq(taa.prev_proj()[i], proj[i]))
            return fail("prev_proj mismatch at index " + std::to_string(i));
    }
