#define WISH_LOG_CATEGORY "ActivityMgr"
...
ActivityId ActivityManager::start_activity(ActivityId template_id,
    ...
    if (!instance->initialize(*tmpl)) {
        wish::log_warning_cat(WISH_LOG_CATEGORY,
            "failed to initialize activity from template " + std::string(tmpl->name));
        return 0;
    }
    ...
    wish::log_info_cat(WISH_LOG_CATEGORY,
        "started activity '" + std::string(tmpl->name) + "' (id=" + std::to_string(tmpl->id) + ")");
    return tmpl->id;
}
...
ActivityId ActivityManager::start_activity(std::string_view name,
    ...
    if (!tmpl) {
        wish::log_warning_cat(WISH_LOG_CATEGORY,
            "unknown template name '" + std::string(name) + "'");
        return 0;
    }
