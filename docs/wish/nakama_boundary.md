...
| **Nakama integration** | HTTP token-validation bridge and feature gating | `wish/integrations/nakama/src/nakama_bridge.cpp` + `wish/include/wish/integrations/nakama/nakama_bridge.h` |
...
`wish/integrations/nakama/mock_session_services.h` still provides:
...
`wish/integrations/nakama/nakama_bridge.h` now also provides:

| Type | Behavior |
|---|---|
| `BridgeSettings` | Loads bridge config from env/CLI |
| `NakamaHttpAuthValidator` | Calls Nakama's HTTP account endpoint with the Bearer session token |
| `make_auth_validator()` | Returns the real HTTP validator when the bridge is enabled, otherwise the no-op validator |

### Dependency wiring (Dedicated server)

`server/src/dedicated_server_main.cpp` now loads `BridgeSettings`, constructs an `AuthValidator`, and then passes the three services through the session lifecycle:
...
When the bridge is disabled, the server uses the existing no-op validator. When `WISH_NAKAMA_ENABLED=1` or `--nakama` is supplied, the server uses `NakamaHttpAuthValidator` instead. No Nakama SDK headers appear in the server or simulation code.

## Future plug-in points

### 1. `AuthValidator::validate()` → validate session token

**Current**: If the bridge is enabled, the validator sends a plain HTTP `GET /v2/account` request to Nakama with `Authorization: Bearer <session-token>`. If Nakama returns `200 OK`, Wish Engine extracts `user.id` from the JSON response and accepts the session. If the bridge is disabled, the existing no-op validator still accepts everything with a placeholder token.
...
- Potentially replace the built-in HTTP bridge with a full Nakama SDK-backed validator if that becomes useful
...
**Code location**: `wish/integrations/nakama/src/nakama_bridge.cpp`
...
The existing `BridgeSettings` struct and `is_enabled()` function in `nakama_bridge.h/cpp` provide a feature flag and connection settings. When the Nakama bridge is disabled (default), the runtime uses no-op implementations. When enabled, the server swaps in the built-in HTTP validator. This keeps local offline workflows simple while allowing direct Wish Engine → Nakama token validation.
...
- The dedicated server wires in a real HTTP auth validator when the bridge is enabled and otherwise falls back to the no-op validator.
