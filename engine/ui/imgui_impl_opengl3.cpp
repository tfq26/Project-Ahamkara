...
bool ImGui_ImplOpenGL3_InitLoader()
{
    ...
    {
#if defined(__APPLE__)
        // On macOS Apple Silicon, the dlsym-based loader in imgl3w cannot
        // resolve Core Profile 3.3 symbols from the OpenGL framework dylib.
        // glfwGetProcAddress (from the dynamically-loaded GLFW library)
        // knows how to query the current GL context for function pointers.
        // We find it via RTLD_DEFAULT since GLFW is already loaded.
        {
            void* glfw_proc = ::dlsym(RTLD_DEFAULT, "glfwGetProcAddress");
            if (glfw_proc != nullptr) {
                imgl3wInit2((GL3WGetProcAddressProc)glfw_proc);
            } else {
                // Fallback: try the default imgl3w loader
                imgl3wInit();
            }
        }
#else
