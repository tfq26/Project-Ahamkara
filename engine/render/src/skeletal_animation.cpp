    ...
    std::vector<Mat4>& out_joint_matrices)
{
    ...
    for (std::size_t j = 0; j < joint_count; ++j) {
        const auto& joint = skin.joints[j];
        if (joint.inverse_bind_matrix.size() >= 16) {
            // Build Mat4 from inverse_bind_matrix (already column-major)
            Mat4 ibm;
            for (int i = 0; i < 16; ++i) {
                ibm.m[static_cast<std::size_t>(i)] = joint.inverse_bind_matrix[static_cast<std::size_t>(i)];
            }
            out_joint_matrices[j] = global_transforms[j] * ibm;
        } else {
            out_joint_matrices[j] = global_transforms[j];
        }
    }
}
