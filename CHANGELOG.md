...
- **Automated release workflow** (`.github/workflows/release.yml`): New GitHub Actions workflow triggered on version tag pushes (`vX.Y.Z`). Builds all CMake presets (debug, release, debug-headless), runs tests, packages TGZ/ZIP archives via CPack, generates release notes from git log, updates `CHANGELOG.md`, and publishes a GitHub Release with artifacts. See `docs/guides/releasing.md` for usage.
