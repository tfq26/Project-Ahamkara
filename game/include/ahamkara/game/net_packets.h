...
inline bool write_header(ByteWriter& writer, PacketType type) {
    PacketHeader header {};
    header.type = type;
    return writer.write(header.magic) && writer.write(header.version) && writer.write(static_cast<ae::u16>(header.type));
}
...
inline bool read_header(ByteReader& reader, PacketType expected_type) {
    ...
    return magic == kPacketMagic && version == kProtocolVersion && type == static_cast<ae::u16>(expected_type);
}
...
inline bool write_player_input(ByteWriter& writer, const PlayerInputCommand& command) {
    return writer.write(command.sequence) && writer.write(command.client_tick) && writer.write(command.client_time) && write_vec2(writer, command.move_axis) && write_vec2(writer, command.look_delta) && writer.write_bool(command.jump_pressed) && writer.write_bool(command.crouch_held) && writer.write_bool(command.sprint_held) && writer.write_bool(command.slide_pressed) && writer.write_bool(command.fire_held) && writer.write_bool(command.reload_pressed) && writer.write_bool(command.ability_pressed) && writer.write_bool(command.interact_pressed);
}

inline bool read_player_input(ByteReader& reader, PlayerInputCommand& command) {
    return reader.read(command.sequence) && reader.read(command.client_tick) && reader.read(command.client_time) && read_vec2(reader, command.move_axis) && read_vec2(reader, command.look_delta) && reader.read_bool(command.jump_pressed) && reader.read_bool(command.crouch_held) && reader.read_bool(command.sprint_held) && reader.read_bool(command.slide_pressed) && reader.read_bool(command.fire_held) && reader.read_bool(command.reload_pressed) && reader.read_bool(command.ability_pressed) && reader.read_bool(command.interact_pressed);
}

inline bool write_player_state(ByteWriter& writer, const ReplicatedPlayerState& state) {
    return writer.write(state.network_object_id) && writer.write(state.player_id) && write_vec3(writer, state.position) && write_vec3(writer, state.velocity) && writer.write(state.yaw) && writer.write(static_cast<ae::u8>(state.movement_state)) && writer.write(state.health) && writer.write(state.shield);
}

inline bool read_player_state(ByteReader& reader, ReplicatedPlayerState& state) {
    ae::u8 movement_state_value = 0;

    if (!reader.read(state.network_object_id) || !reader.read(state.player_id) || !read_vec3(reader, state.position) || !read_vec3(reader, state.velocity) || !reader.read(state.yaw) || !reader.read(movement_state_value) || !reader.read(state.health) || !reader.read(state.shield)) {
        return false;
    }
...
inline bool write_snapshot(ByteWriter& writer, const ServerSnapshot& snapshot) {
    ...
    for (ae::u8 i = 0; i < snapshot.projectile_count; ++i) {
        ...
        if (!write_vec3(writer, p.position) || !write_vec3(writer, p.velocity) || !writer.write(p.lifetime_seconds) || !writer.write_bool(p.alive) || !writer.write(p.client_tick))
            return false;
    }
    if (!writer.write(snapshot.dummy_count))
        return false;
    for (ae::u8 i = 0; i < snapshot.dummy_count; ++i) {
        const auto& d = snapshot.dummies[i];
        if (!writer.write(d.dummy_id) || !write_vec3(writer, d.position) || !writer.write(d.yaw) || !writer.write(d.health) || !writer.write_bool(d.alive))
            return false;
    }
    if (!writer.write(snapshot.match_phase) || !writer.write(snapshot.match_time) || !writer.write(snapshot.team_score_red) || !writer.write(snapshot.team_score_blue) || !writer.write(snapshot.individual_score) || !writer.write(snapshot.remote_player_count))
        return false;
...
inline bool read_snapshot(ByteReader& reader, ServerSnapshot& snapshot) {
    if (!reader.read(snapshot.server_tick) || !reader.read(snapshot.last_processed_input) || !read_player_state(reader, snapshot.local_player) || !reader.read(snapshot.projectile_count))
        return false;
    if (snapshot.projectile_count > 8)
        return false;
    for (ae::u8 i = 0; i < snapshot.projectile_count; ++i) {
        auto& p = snapshot.projectiles[i];
        if (!read_vec3(reader, p.position) || !read_vec3(reader, p.velocity) || !reader.read(p.lifetime_seconds) || !reader.read_bool(p.alive) || !reader.read(p.client_tick))
            return false;
    }
    if (!reader.read(snapshot.dummy_count) || snapshot.dummy_count > 4)
        return false;
    for (ae::u8 i = 0; i < snapshot.dummy_count; ++i) {
        auto& d = snapshot.dummies[i];
        if (!reader.read(d.dummy_id) || !read_vec3(reader, d.position) || !reader.read(d.yaw) || !reader.read(d.health) || !reader.read_bool(d.alive))
            return false;
    }
    if (!reader.read(snapshot.match_phase) || !reader.read(snapshot.match_time) || !reader.read(snapshot.team_score_red) || !reader.read(snapshot.team_score_blue) || !reader.read(snapshot.individual_score) || !reader.read(snapshot.remote_player_count))
        return false;
    if (snapshot.remote_player_count > 4)
        return false;
    for (ae::u8 i = 0; i < snapshot.remote_player_count; ++i) {
...
inline bool write_client_hello(ByteWriter& writer, const ClientHelloPacket& packet) {
    return writer.write(packet.protocol_version) && writer.write(packet.auth_token_length) && writer.write_bytes(packet.auth_token, packet.auth_token_length);
}

inline bool read_client_hello(ByteReader& reader, ClientHelloPacket& packet) {
    if (!reader.read(packet.protocol_version) || !reader.read(packet.auth_token_length))
        return false;
    if (packet.auth_token_length > kMaxAuthTokenLength)
        return false;
...
inline bool write_server_welcome(ByteWriter& writer, const ServerWelcomePacket& packet) {
    return writer.write(packet.protocol_version) && writer.write_bytes(packet.player_id, kMaxPlayerIdLength);
}

inline bool read_server_welcome(ByteReader& reader, ServerWelcomePacket& packet) {
    return reader.read(packet.protocol_version) && reader.read_bytes(packet.player_id, kMaxPlayerIdLength);
}

inline bool write_server_reject(ByteWriter& writer, const ServerRejectPacket& packet) {
    return writer.write(packet.protocol_version) && writer.write(static_cast<ae::u8>(packet.reason));
}

inline bool read_server_reject(ByteReader& reader, ServerRejectPacket& packet) {
    ae::u8 reason_value = 0;
    if (!reader.read(packet.protocol_version) || !reader.read(reason_value) || reason_value == 0U || reason_value > static_cast<ae::u8>(HandshakeRejectReason::ServerBusy)) {
        return false;
    }
...
inline bool write_envelope(ByteWriter& writer, const PacketEnvelope& envelope) {
    return writer.write(envelope.sequence) && writer.write(envelope.ack_sequence) && writer.write(envelope.ack_bitfield);
}

inline bool read_envelope(ByteReader& reader, PacketEnvelope& envelope) {
    return reader.read(envelope.sequence) && reader.read(envelope.ack_sequence) && reader.read(envelope.ack_bitfield);
}

inline bool write_heartbeat(ByteWriter& writer, const HeartbeatPacket& packet) {
    return writer.write(packet.server_tick) && writer.write(packet.connected_players);
}

inline bool read_heartbeat(ByteReader& reader, HeartbeatPacket& packet) {
    return reader.read(packet.server_tick) && reader.read(packet.connected_players);
}

inline bool write_client_reconnect(ByteWriter& writer, const ClientReconnectPacket& packet) {
    return writer.write(packet.session_id) && writer.write(packet.protocol_version);
}

inline bool read_client_reconnect(ByteReader& reader, ClientReconnectPacket& packet) {
    return reader.read(packet.session_id) && reader.read(packet.protocol_version);
}
...
inline bool write_snapshot_delta(detail::ByteWriter& writer, const SnapshotDelta& d) {
    if (!writer.write(d.server_tick) || !writer.write(d.last_processed_input) || !writer.write(d.dirty_mask))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosX)))
        writer.write(d.pos_x);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosY)))
        writer.write(d.pos_y);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosZ)))
        writer.write(d.pos_z);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelX)))
        writer.write(d.vel_x);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelY)))
        writer.write(d.vel_y);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelZ)))
        writer.write(d.vel_z);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Yaw)))
        writer.write(d.yaw);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Movement)))
        writer.write(d.movement);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Health)))
        writer.write(d.health);
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Shield)))
        writer.write(d.shield);
    return true;
}
...
inline bool read_snapshot_delta(detail::ByteReader& reader, SnapshotDelta& d) {
    if (!reader.read(d.server_tick) || !reader.read(d.last_processed_input) || !reader.read(d.dirty_mask))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosX)) && !reader.read(d.pos_x))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosY)) && !reader.read(d.pos_y))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::PosZ)) && !reader.read(d.pos_z))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelX)) && !reader.read(d.vel_x))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelY)) && !reader.read(d.vel_y))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::VelZ)) && !reader.read(d.vel_z))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Yaw)) && !reader.read(d.yaw))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Movement)) && !reader.read(d.movement))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Health)) && !reader.read(d.health))
        return false;
    if (d.dirty_mask & (1 << static_cast<ae::u16>(PlayerDeltaBit::Shield)) && !reader.read(d.shield))
        return false;
    return true;
}
...
static constexpr ae::usize kMinClientHelloWireSize = sizeof(ae::u32) + sizeof(ae::u16) + sizeof(ae::u16) + kEnvelopeWireSize + sizeof(ae::u16) + sizeof(ae::u16);
...
inline bool serialize_player_input_packet(
    ...
    return detail::write_header(writer, PacketType::PlayerInput) && detail::write_envelope(writer, envelope) && detail::write_player_input(writer, command) && writer.bytes_written() == buffer.size();
}
...
inline bool deserialize_player_input_packet(
    ...
    return detail::read_header(reader, PacketType::PlayerInput) && detail::read_envelope(reader, envelope) && detail::read_player_input(reader, command) && reader.is_complete();
}
...
inline bool serialize_server_snapshot_packet(
    ...
    bool ok = detail::write_header(writer, PacketType::ServerSnapshot) && detail::write_envelope(writer, envelope) && detail::write_snapshot(writer, snapshot);
    if (ok && out_bytes_written)
        *out_bytes_written = writer.bytes_written();
    return ok;
}
...
inline bool deserialize_server_snapshot_packet(
    ...
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::ServerSnapshot) && detail::read_envelope(reader, envelope) && detail::read_snapshot(reader, snapshot);
}
...
inline bool serialize_client_hello_packet(
    ...
    ae::usize* out_bytes_written = nullptr) {
    detail::ByteWriter writer(std::span<std::byte>(buffer.data(), buffer.size()));
    bool ok = detail::write_header(writer, PacketType::ClientHello) && detail::write_envelope(writer, envelope) && detail::write_client_hello(writer, packet);
    if (ok && out_bytes_written)
        *out_bytes_written = writer.bytes_written();
    return ok;
}
...
inline bool deserialize_client_hello_packet(
    ...
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::ClientHello) && detail::read_envelope(reader, envelope) && detail::read_client_hello(reader, packet);
}
...
inline bool serialize_server_welcome_packet(
    ...
    detail::ByteWriter writer(buffer);
    return detail::write_header(writer, PacketType::ServerWelcome) && detail::write_envelope(writer, envelope) && detail::write_server_welcome(writer, packet) && writer.bytes_written() == buffer.size();
}
...
inline bool deserialize_server_welcome_packet(
    ...
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::ServerWelcome) && detail::read_envelope(reader, envelope) && detail::read_server_welcome(reader, packet) && reader.is_complete();
}
...
inline bool serialize_server_reject_packet(
    ...
    detail::ByteWriter writer(buffer);
    return detail::write_header(writer, PacketType::ServerReject) && detail::write_envelope(writer, envelope) && detail::write_server_reject(writer, packet) && writer.bytes_written() == buffer.size();
}
...
inline bool deserialize_server_reject_packet(
    ...
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::ServerReject) && detail::read_envelope(reader, envelope) && detail::read_server_reject(reader, packet) && reader.is_complete();
}
...
inline bool serialize_heartbeat_packet(
    ...
    detail::ByteWriter writer(buffer);
    return detail::write_header(writer, PacketType::Heartbeat) && detail::write_envelope(writer, envelope) && detail::write_heartbeat(writer, packet) && writer.bytes_written() == buffer.size();
}
...
inline bool deserialize_heartbeat_packet(
    ...
    if (buffer.size() != heartbeat_packet_size())
        return false;
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::Heartbeat) && detail::read_envelope(reader, envelope) && detail::read_heartbeat(reader, packet) && reader.is_complete();
}
...
inline bool serialize_client_reconnect_packet(
    ...
    detail::ByteWriter writer(buffer);
    return detail::write_header(writer, PacketType::ClientReconnect) && detail::write_envelope(writer, envelope) && detail::write_client_reconnect(writer, packet) && writer.bytes_written() == buffer.size();
}
...
inline bool deserialize_client_reconnect_packet(
    ...
    if (buffer.size() != client_reconnect_packet_size())
        return false;
    detail::ByteReader reader(buffer);
    return detail::read_header(reader, PacketType::ClientReconnect) && detail::read_envelope(reader, envelope) && detail::read_client_reconnect(reader, packet) && reader.is_complete();
}
